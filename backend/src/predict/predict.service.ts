...
export class PredictService {
  ...
  async predict(buildingId: string): Promise<void> {
    try {
      ...
      if (!building) {
        this.logger.warn(`Building ${buildingId} not found`);
        return;
      }
      ...
      const result = await this.runPython(JSON.stringify(buildingJson));
